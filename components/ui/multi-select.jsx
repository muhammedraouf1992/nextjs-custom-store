...
const MultiSelector = ({
  ...
  const onValueChangeHandler = useCallback(
    (val) => {
      if (value.some((item) => item.id === val.id)) {
        onValueChange(value.filter((item) => item.id !== val.id));
      } else {
        onValueChange([...value, val]);
      }
    },
...
const MultiSelectorItem = forwardRef(
  ({ className, value: item, children, ...props }, ref) => {
    const { value: Options, onValueChange, setInputValue } = useMultiSelect();
    ...
    const isIncluded = Options.some((selected) => selected.id === item.id);
    return (
      <CommandItem
        ref={ref}
        {...props}
